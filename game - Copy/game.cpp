...
// initial position of the ship will be approx middle of screen
	...
	while (window.isOpen())
	{
		...
		if (start.getIsClicked() == false)
		{
			...
			if ( anAliens.getAlien().getAliensSprite().getPosition().y <1600)
			{
				
				isPlaying = false;
				Font title1;
				
				if (!title1.loadFromFile("C:\\windows\\Fonts\\arial.ttf"))
				{
					//	die ("couldn't load font");
				}
				Text title("Game Over", title1,300);
				title.setPosition(window.getSize().x / 2.0f, window.getSize().x / 2.0f);
				window.draw(title);
